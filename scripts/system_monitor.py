            ...
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent', 'create_time']):
                try:
                    ...
                    if (proc.info['name'] == 'python' and
                        proc.info['cpu_percent'] > 80 and
                        time.time() - proc.info['create_time'] > 1800):  # 30 минут
    ...
    async def monitor_loop(self):
        ...
        while True:
            try:
                ...
                if stats:
                    logger.info(f"📊 Система: CPU {stats['cpu_percent']}%, RAM {stats['memory_percent']}% "
                    ...
                    for proc in stats.get('python_processes', []):
                        logger.info(f"  🐍 PID {proc['pid']}: CPU {proc['cpu']:.1f}%, RAM {proc['memory']:.1f}%, "
                                  f"CMD: {proc['cmdline']}")
                ...
                if queue_stats:
                    for queue_name, stat in queue_stats.items():
                        logger.info(f"📋 Очередь {queue_name}: {stat['messages']} сообщений, "
                                  f"{stat['consumers']} потребителей")
