...
async def test_get_schedule_image_generate(image_service, mock_cache_manager, mocker):
    ...
    mocker.patch.object(
        ...
        AsyncMock(return_value=(True, "generated.png")),
    )
    success, file_path = await image_service.get_or_generate_week_image("TEST_GROUP", "odd", "Нечетная", {})
    assert success
...
async def test_send_image_to_user_success(image_service, mocker, tmp_path):
    ...
    mocker.patch("pathlib.Path.exists", return_value=True)
    mocker.patch(
        "bot.utils.image_compression.get_telegram_safe_image_path",
        return_value=str(image_file),
    )
    mocker.patch.object(
        image_service.bot,
        "edit_message_media",
        AsyncMock(side_effect=Exception("test exception to trigger send")),
    )
