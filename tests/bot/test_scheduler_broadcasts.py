...
@pytest.fixture
def fake_timetable(mocker):
    tt = MagicMock()
    mocker.patch.object(scheduler_module, 'global_timetable_manager_instance', tt)
    return tt


@pytest.mark.asyncio
async def test_evening_broadcast_no_users(fake_metrics, fake_send_task, fake_weather):
    udm = AsyncMock()
    udm.get_users_for_evening_notify.return_value = []
    await evening_broadcast(udm)
...
async def test_evening_broadcast_with_users_and_no_lessons(fake_metrics, fake_send_task, fake_weather, fake_timetable, mocker):
    ...
    # зафиксируем дату, чтобы избежать флаки
...
@pytest.mark.asyncio
async def test_morning_summary_no_users(fake_metrics, fake_send_task, fake_weather, mocker):
    udm = AsyncMock()
...
async def test_monitor_schedule_changes_detected(fake_metrics, fake_send_task, mocker):
    # старый и новый хеш различаются
    redis = AsyncMock()
    redis.get.return_value = b'old'
    udm = AsyncMock()
    udm.get_all_user_ids.return_value = [1, 2]

    # подготовим новые данные расписания
    new_data = {
        '__metadata__': {},
        '__teachers_index__': {},
        '__classrooms_index__': {},
        '__current_xml_hash__': 'new',
        'G-1': {'odd': {}, 'even': {}},
    }
    ...
    # Проверяем, что среди вызовов был set для ключа хеша расписания
    assert any(
        c.args == ('timetable:schedule_hash', 'new')
...
async def test_collect_db_metrics_sets_gauges(fake_metrics):
    ...
    # если исключений нет — считаем успехом
