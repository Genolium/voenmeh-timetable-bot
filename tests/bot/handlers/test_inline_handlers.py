...
from bot.handlers.inline_handlers import inline_query_handler, parse_day_from_query


@pytest.mark.parametrize(
    "query, expected_day_offset, remaining_parts",
    [
...
class TestInlineHandler:
    ...
    def mock_manager(self):
        ...
        manager.get_schedule_for_day = AsyncMock(
            return_value={
                "date": date.today(),
    ...
    async def test_inline_query_success(self, mock_query, mock_manager, mocker):
        mocker.patch(
            "bot.handlers.inline_handlers.format_schedule_text",
    ...
    async def test_inline_query_no_lessons(self, mock_query, mock_manager, mocker):
        mocker.patch(
            "bot.handlers.inline_handlers.format_schedule_text",
            return_value="Formatted Text No Lessons",
        )
