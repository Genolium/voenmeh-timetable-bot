...
async def on_group_entered(message: Message, message_input: MessageInput, manager: DialogManager):
    ...
    if group_name not in all_groups:
        ...
        if good_suggestions:
            ...
            await message.answer(f"🔍 <b>Группа \"{group_name}\" не найдена точно</b>\n\n"
...
dialog = Dialog(
    ...
    Window(
        StaticMedia(path=WELCOME_IMAGE_PATH),
        Format("🎓 <b>Регистрация студента</b>\n\n"
        ...
        Button(Const("⬅️ Назад к выбору роли"), id="back_to_role", on_click=lambda c, b, m: m.switch_to(MainMenu.choose_user_type)),
    ...
    Window(
        StaticMedia(path=WELCOME_IMAGE_PATH),
        Const("🧑‍🏫 <b>Регистрация преподавателя</b>\n\n"
        ...
        Button(Const("⬅️ Назад к выбору роли"), id="back_to_role_teacher", on_click=lambda c, b, m: m.switch_to(MainMenu.choose_user_type)),
    ...
    Window(
        ...
        Row(
            ...
            Button(Const("🚀 Начать пользоваться!"), id=WidgetIds.SKIP_TUTORIAL, on_click=on_skip_tutorial_clicked)
        ),
