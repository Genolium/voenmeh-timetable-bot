...
class WeatherAPI:
    ...
    async def get_forecast_for_time(self, target_datetime: datetime) -> Optional[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                logging.info(f"Выполняю запрос к OpenWeatherMap API для {cache_key}...")
                ...
                if closest_forecast_item:
                    ...
                    self._cache[cache_key] = {'timestamp': datetime.now(timezone.utc), 'data': result_data}
                    return result_data
                return None
                ...
                logging.error(f"Неизвестная ошибка при получении прогноза погоды: {e}")
                return None
