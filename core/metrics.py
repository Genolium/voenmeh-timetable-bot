...
USERS_TOTAL = Gauge("bot_users_total", "Total number of users in the database")

# Датчик для количества пользователей с активными подписками на рассылки.
SUBSCRIBED_USERS = Gauge("bot_subscribed_users", "Number of users with at least one active subscription")
...
HANDLER_DURATION = Histogram("bot_handler_duration_seconds", "Duration of handler processing", ["handler_name"])

# Счетчик для задач, отправленных в очередь Dramatiq.
# 'actor_name' - метка для идентификации конкретного актора Dramatiq.
TASKS_SENT_TO_QUEUE = Counter("bot_tasks_sent_total", "Total count of tasks sent to the queue", ["actor_name"])

# Общие ошибки по источникам (handler, parser, weather, db, tasks)
ERRORS_TOTAL = Counter("bot_errors_total", "Total count of errors by source", ["source"])
...
USER_ACTIVITY_DAILY = Counter(
    ...
    [
        ...
        "user_group",
        "hour_of_day",
    ],  # view_schedule, search, settings, etc.
)
...
USER_ACTIONS = Counter(
    ...
    [
        ...
        "user_type",
        "source",
    ],  # menu_click, button_click, command, callback, etc.
)
...
ACTIVE_SESSIONS = Gauge("bot_active_sessions", "Number of active user sessions", ["user_type"])

# Команды бота
BOT_COMMANDS = Counter("bot_commands_total", "Total count of bot commands used", ["command", "user_type"])
...
# Время ответа API
API_RESPONSE_TIME = Histogram("bot_api_response_time_seconds", "API response time", ["api_endpoint", "method"])
...
RESTART_COUNT = Counter("bot_restart_count_total", "Total number of bot restarts")
...
NOTIFICATION_DELIVERY = Counter(
    ...
    [
        "delivery_status",
        "notification_type",
    ],  # success, failed, schedule, weather, reminder
)
...
# Размер БД
DATABASE_SIZE = Gauge("bot_database_size_bytes", "Database size in bytes", ["table_name"])
